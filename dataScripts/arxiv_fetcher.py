...
def fetch_papers(keyword, max_results=10):
    ...
    papers = []
    for result in tqdm(
        client.results(search), desc="Fetching papers...", total=max_results
    ):
        papers.append(
            {
                "title": result.title,
                ...
                "pdf_url": next(
                    filter(lambda item: item.title == "pdf", result.links)
                ).href,
            }
        )
    return papers
...
if __name__ == "__main__":
    ...
    for paper in tqdm(papers, desc="Downloading papers..."):
        save_path = os.path.join(docs_dir, re.sub(r"[:?,]", r"_", paper["title"]) + ".pdf")
        download_pdf(
            paper["pdf_url"],
            save_path=save_path
        )

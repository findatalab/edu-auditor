...
engine = create_engine(
    f"postgresql://postgres:{DB_PASSWORD}@localhost/prototypedocumentreviewsystem"
)
...
class FaissLoader:
    ...
    def load_and_save_database(self, folder_path: str, save_to: str, engine) -> None:
        ...
        index = faiss.IndexFlatIP(
            self.embedding_class.model.embeddings.word_embeddings.weight.shape[1]
        )
        ...
        for pdf_filename in tqdm(pdf_filenames, desc="Going through filenames..."):
            ...
            for chunk_batch in embedded_chunks_yielding:
                ...
                clean_slice_chunks = [
                    chunk.replace("\x00", "") for chunk in slice_chunks
                ]
                chunk_ids = list(
                    range(
                        previous_total + CURRENT_TOTAL_VECTORS,
                        next_total + CURRENT_TOTAL_VECTORS,
                    )
                )
                db_chunks = [
                    Chunk(id=id, chunk=chunk)
                    for id, chunk in zip(chunk_ids, clean_slice_chunks)
                ]
...
if __name__ == "__main__":
    embedding_class = TextEmbedding(
        model_name="Snowflake/snowflake-arctic-embed-l-v2.0", device="cpu"
    )
    faiss_loader = FaissLoader(embedding_class=embedding_class)
    faiss_loader.load_and_save_database(
        folder_path="C:\\main\\Git\\uni_practice\\edu-auditor\\DataDocs",
